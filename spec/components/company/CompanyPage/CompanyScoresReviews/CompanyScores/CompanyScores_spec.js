...
describe('CompanyScores', () => {
  ...
  describe('renders company scores', () => {
    ...
    it('renders review count when 0', () => {
      render.setProps({
        reviewCount: 0
      });
      const text = render.find('.review-count').first().find('FormattedMessage').dive()
        .text();
      chai.expect(text).to.equal('No reviews yet');
    });
    ...
    it('renders review count more than 1', () => {
      render.setProps({
        reviewCount: 10
      });
      const text = render.find('.review-count').first().find('FormattedMessage').dive()
  ...
  describe('renders review percentage bars', () => {
    ...
    describe('renders percentages when review count is 0', () => {
      before(() => {
        render.setProps({
          reviewCount: 0
        });
  ...
  describe('renders aspects', () => {
    ...
    it('does not render if no reviews', () => {
      render.setProps({
        reviewCount: 0
      });
      chai.expect(render.find('.total-aspects')).to.have.length(0);
